...
import { sliceCommand, appendSplitToParent, splitProposalIntent } from '../commands/slice.js';
...
const decomposition = {
  children: [
    { specId: '030-a', title: 'Foundation A', scope: 'covers FR-001', assignedRequirementIds: ['FR-001'], dependsOn: [], rice: { reach: 5, impact: 5, confidence: 1, effort: 1 } },
    { specId: '031-b', title: 'Builds on A', scope: 'covers FR-002, FR-003', assignedRequirementIds: ['FR-002', 'FR-003'], dependsOn: ['030-a'], rice: { reach: 2, impact: 2, confidence: 1, effort: 1 } },
  ],
};

describe('sliceCommand', () => {
  it('emits a <spec-split> with children in R-002 order and a split verdict', async () => {
    const ai = sliceStub({ decompositions: [decomposition], confirmations: [{ RICE: 'Accept' }] });
    const res = await sliceCommand({ parentSpecId: '029-value-ranked-slicer', parentHtml, runCritic: false }, { cwd: '.', ai });
