...
/**
 * Unit tests for designCommand. Stub AIProvider; no network; no
...
<spec-questions><ol><li>What should X do when Y?</li></ol></spec-questions>
    ...
    const result = await designCommand(baseInput({ specHtml }), ctxFrom(ai));
...
<spec-out-of-scope><ul><li>Dark mode polish</li></ul></spec-out-of-scope>
  ...
  it('happy path: returns rendered html with decision + principles counts', async () => {
    ...
    // open-time CSP gate too, not just the file-based templates/design.html.
  ...
  it('throws when AI marks any principle as VIOLATION', async () => {
    ...
    await expect(designCommand(baseInput(), ctxFrom(ai))).rejects.toThrow(/1 principle\(s\) marked VIOLATION/);
  });

  it('re-entry mode: prompt includes "Sharpen this design" and changelog reflects re-entry', async () => {
    const ai = new StubAI([
      {
        json: {
    ...
    const existingDesign = `<!doctype html><html><body>
<spec-decision id="D-001"><h4>D-001 · Existing</h4></spec-decision>
</body></html>`;

    const result = await designCommand(baseInput({ existingDesign }), ctxFrom(ai));
  ...
  it('throws when ctx.ai is undefined', async () => {
    await expect(designCommand(baseInput(), { cwd: '/tmp/test' })).rejects.toThrow(/designCommand requires ctx\.ai/);
  });
});
