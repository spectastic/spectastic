...
function fixture(files: Record<string, string>): string {
  ...
  for (const [name, body] of Object.entries(files)) {
    ...
    mkdirSync(dirname(target), { recursive: true }); // supports a nested signal path like .github/dependabot.yml
...
describe('detectTooling: per-ecosystem classification', () => {
  ...
  // 068 dogfooding finding: biome.json is strict JSON and can't carry the
  // inline rule-rationale comments FR-002 requires; biome.jsonc is Biome's
  // own sanctioned comment-carrying variant (this repo's own biome.jsonc).
  // Without this signal row, spectastic's own `enforce` would report a false
  // formatter/linter gap despite Biome being fully configured and wired.
  it('JS/TS: biome.jsonc (the comment-carrying config variant) also classifies', () => {
    const dir = fixture({ 'biome.jsonc': '{\n  // a recorded rule rationale\n}\n' });
    const c = detectTooling(dir);
    expect(c.has('formatter')).toBe(true);
    expect(c.has('linter')).toBe(true);
  });
  ...
  // 068-enterprise-enforce-floor T-300/T-301 (plan D-004). This own repo carries
  // .github/dependabot.yml (real, checked in) but until now the detector had no
  // signal row for it — a false negative on its own supply-chain enforcement.
  it('Dependabot is recognised as supply-chain (T-300)', () => {
    const dir = fixture({ '.github/dependabot.yml': "version: 2\nupdates:\n  - package-ecosystem: 'npm'\n" });
    const c = detectTooling(dir);
    expect(c.has('supply-chain')).toBe(true);
  });

  it('a Renovate-covered project stays covered — additive regression guard (T-301, SC-004)', () => {
    // Adding the Dependabot row must not disturb the pre-existing Renovate signal.
    const dir = fixture({ 'renovate.json': '{}' });
    const c = detectTooling(dir);
    expect(c.has('supply-chain')).toBe(true);
  });
});
...
describe('detectTooling: coverage is threshold-bearing, not bare-presence', () => {
  ...
  it('Java: JaCoCo verification task in build.gradle → coverage', () => {
    const dir = fixture({
      'build.gradle': 'jacocoTestCoverageVerification {\n  violationRules { rule { limit { minimum = 0.9 } } }\n}\n',
    });
    expect(detectTooling(dir).has('coverage')).toBe(true);
  });
  ...
  it('a bare coverage library with no declared threshold is NOT classified as coverage', () => {
    ...
    const dir = fixture({
      'package.json': '{"devDependencies":{"@vitest/coverage-v8":"^2.0.0"}}',
    });
...
describe('detectTooling: observability is an exporter dependency, not a core/tracing lib', () => {
  ...
  it('JS/TS: @opentelemetry/exporter-prometheus in package.json → observability', () => {
    const dir = fixture({
      'package.json': '{"dependencies":{"@opentelemetry/exporter-prometheus":"^0.52.0"}}',
    });
    expect(detectTooling(dir).has('observability')).toBe(true);
  });
  ...
  it('a tracing-only core lib (@opentelemetry/api) is NOT classified as observability (adversarial-pass Risk 1)', () => {
    const dir = fixture({
      'package.json': '{"dependencies":{"@opentelemetry/api":"^1.9.0"}}',
    });
  ...
  it('a bare micrometer-core with no registry is NOT classified as observability', () => {
    const dir = fixture({
      'build.gradle': "implementation 'io.micrometer:micrometer-core'\n",
    });
    expect(detectTooling(dir).has('observability')).toBe(false);
  });
});
