...
describe('score', () => {
  it('weights: high=40, medium=15, low=5', () => {
    ...
    expect(score([finding('medium')], {})).toEqual({ score: 15, band: 'green' });
  ...
  it('two highs land in the red band by default (80)', () => {
    expect(score([finding('high'), finding('high')], {})).toEqual({ score: 80, band: 'red' });
  });
  ...
  it('respects default band boundaries (amber inclusive on both ends)', () => {
    expect(score([finding('medium'), finding('low')], {})).toEqual({ score: 20, band: 'green' }); // 20 < 25
  ...
  it('uses configured bands over the shipped defaults', () => {
    ...
    expect(score(findings, { bands: { amber: 10, red: 50 } })).toMatchObject({ band: 'amber' });
  });
});
