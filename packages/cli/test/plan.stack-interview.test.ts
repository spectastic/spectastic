 ...
 * commands/spectastic.plan.md (spec 050-stack-selection, US1/US2).
 *
 * AskUserQuestion is a harness-native interactive tool with no CLI/AI-stub
 * -testable surface — packages/core/src/commands/plan.ts's AI contract is a
...
// commands/spectastic.plan.md is the source of truth (CLAUDE.md: .claude/commands/
...
const PLAN_MD = resolve(here, '..', '..', '..', 'commands', 'spectastic.plan.md');
...
describe('stack-selection interview — decision-phase instructions (US1)', () => {
  ...
  it('is gated by plan.stackInterview, on by default (FR-005)', () => {
    const md = content();
    expect(md).toMatch(/plan\.stackInterview/);
