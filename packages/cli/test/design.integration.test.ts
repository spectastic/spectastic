 ...
 * `spectastic design` subcommand. State-gate per P-6 / FR-003: Draft
...
function setupSpecDir(opts: { planStatus?: string }): {
  ...
  const planPath = join(specDir, 'design.html');
  if (opts.planStatus) {
    writeFileSync(
      planPath,
...
describe('CLI integration: plan (T-112)', () => {
  it('refuses past-Draft design.html with exit 2 — silent-sharpen failure mode closed', async () => {
    const { cwd, specId, planPath } = setupSpecDir({ planStatus: 'accepted' });

    const r = await runCLI(['design', specId], cwd);
  ...
  it('auto-re-enters Draft design.html — gate emits "Auto-re-entering Draft" then AI', async () => {
    const { cwd, specId } = setupSpecDir({ planStatus: 'draft' });

    const r = await runCLI(['design', specId], cwd, { ANTHROPIC_API_KEY: '' });
  ...
  it('--force on past-Draft bypasses with warning', async () => {
    ...
    const r = await runCLI(['design', specId, '--force'], cwd, {
      ANTHROPIC_API_KEY: '',
    });
  ...
  it('fresh path (no design.html) proceeds past the gate', async () => {
  ...
  it('happy path with SPECTASTIC_AI_STUB writes a complete design.html (T-112)', async () => {
