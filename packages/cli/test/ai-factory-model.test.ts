...
import { resolveProviderModel, createAIProvider } from '../src/ai-factory.js';

describe('CLI model resolution (spec 044 Tier D · FR-006/007/008)', () => {
  let dir: string;
  const saved = { stub: process.env['SPECTASTIC_AI_STUB'], model: process.env['SPECTASTIC_MODEL'], key: process.env['ANTHROPIC_API_KEY'] };
  beforeEach(() => {
  ...
  afterEach(() => {
    ...
    for (const [k, v] of Object.entries({ SPECTASTIC_AI_STUB: saved.stub, SPECTASTIC_MODEL: saved.model, ANTHROPIC_API_KEY: saved.key })) {
  ...
  it('the resolved model reaches ClaudeProvider.model (the Assisted-by source)', async () => {
    ...
    const ai = await createAIProvider({ verb: 'implement', override: 'opus', cwd: dir });
