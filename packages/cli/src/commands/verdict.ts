...
export function registerVerdict(program: Command): void {
  program
    ...
    .option('--explain', 'after the verdict, show each violation reviewer-grade: the offending code, the decision, and the sanctioned path (advisory, output-only)')
    .action(async (opts: { changed?: string[]; enforcerOutput?: string; out: string; json?: boolean; teach?: boolean; explain?: boolean }) => {
      const [{ verdictCommand, shifuQuestions, explainViolations, renderExplanations, loadDecisions }, { nodeFs }, fsp] =
        await Promise.all([
