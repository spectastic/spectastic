...
test.describe('FR-009 · vivid header', () => {
  test('vivid shows a sticky top header; calm shows none', async ({ page }) => {
    await page.goto(FIXTURE);
    await setTheme(page, 'spectastic-vivid');
    const bar = page.locator(BAR);
    await expect(bar).toBeVisible();
    expect(await bar.evaluate((el) => getComputedStyle(el).position)).toBe('sticky');
    // It is the first thing in the body (source order = reading order, P-1).
    expect(await bar.evaluate((el) => el.parentElement?.tagName)).toBe('BODY');
    await setTheme(page, 'spectastic-calm');
    await expect(bar).toBeHidden();
  });

  test('the header carries brand→index.html, path, theme dropdown, and mode toggle', async ({ page }) => {
    await page.goto(FIXTURE);
    await setTheme(page, 'spectastic-vivid');
  ...
  for (const mode of ['light', 'dark']) {
    test(`header holds AA contrast · vivid · ${mode}`, async ({ page }) => {
      await page.goto(FIXTURE);
      await setTheme(page, 'spectastic-vivid', mode);
      ...
      expect(offenders, `header contrast failures:\n${offenders.join('\n')}`).toEqual([]);
    });
  }
});
