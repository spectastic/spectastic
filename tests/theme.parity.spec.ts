...
// SC-006 — spectastic-heavy visual parity with the reference design.
...
async function heavy(page) {
  await page.goto(FIXTURE);
  await page.evaluate(() => {
    document.documentElement.setAttribute('data-theme', 'spectastic-heavy');
...
test.describe('SC-006 · heavy parity — Slice 1 (shape)', () => {
...
test.describe('SC-006 · heavy parity — Slice 2 (layout)', () => {
  ...
  test('heavy is one column — top-level blocks share width and left edge', async ({ page }) => {
    await heavy(page);
...
test('NFR-005 — theme styling is attribute-scoped, no document markup', () => {
  ...
  const allowed = ['[data-theme="spectastic-heavy"]', '[data-theme="spectastic-calm"]'];
  ...
  expect(body).not.toMatch(/class="[^"]*spectastic-(heavy|calm)/);
});
