...
const corpus = [
  {
    specId: '001-foundation',
    html: htmlFor('001-foundation', {
      deferTo: ['002-big'],
      rice: [4, 4, 1, 2],
    }),
  },
  {
    specId: '002-big',
    html: htmlFor('002-big', {
      parent: '001-foundation',
      rice: [10, 10, 1, 1],
    }),
  },
...
test('the elevated foundation appears ahead of the higher-own-value leaf (FR-004)', async ({ page }) => {
  await page.goto(URL);
  const ids = (await page.locator('#order tbody tr td:nth-child(2)').allTextContents()).map(
    (t) => t.trim().split(/\s/)[0],
  );
...
test('the WSJF-divergence mark is visible where RICE and WSJF disagree (FR-008)', async ({ page }) => {
  await page.goto(URL);
  const foundationRow = page.locator('#order tbody tr', {
    hasText: '001-foundation',
  });
