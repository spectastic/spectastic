...
for (const rel of files) {
  ...
  if (src.includes('assets/theme-boot.js')) {
    skipped++;
    continue;
  }
  const m = src.match(LINK_RE);
  if (!m) {
    console.warn('  no spec.css link:', rel);
    missing++;
    continue;
  }
...
console.log(
  `retrofit: ${changed} changed · ${skipped} already had it · ${missing} missing link · ${files.length} total`,
);
