...
**Everything here now ships.** The bundle exercises all 23 elements of the visual vocabulary —
[`093`](../../specs/093-design-visual-section/spec.html) through
[`105`](../../specs/105-design-source-import/spec.html) — with at least one worked use each: a
materialised view generated by the real command, a render bound to the grid cell it is evidence of, a
journey with its failure branches, a component with its nine interaction states and three declines, a
motion sequence with its reduced-motion record, copy budgets and refusals, and a tracking plan gated on
an unanswered consent question.

Parity is the point rather than completeness for its own sake. Extending this bundle to cover the whole
vocabulary immediately found a defect nothing else had: `<spec-transition on="…">` was an error-severity
violation of the rule that forbids executable content, because that rule flags every attribute whose name
begins with `on` — correctly, since it cannot tell a vocabulary attribute from `onclick`. The unit tests
and fixtures had used the same attribute and passed, because neither runs the security scan. The
attribute is now `trigger=`.
...
- **The one render is an SVG, not a raster export.** It diffs, it validates, and it suits a repository
  of text artifacts. A real project's renders would be PNG exports from its design tool — the
  substitution is a convenience here and not a recommendation. It is committed under `visual/`, where
  the repository's root-anchored `/*.png` rule does not reach.
