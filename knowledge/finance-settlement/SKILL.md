---
name: finance-settlement
description: Securities-settlement and cash-accounting domain knowledge — trade-date vs. settlement-date timing, clearing-house cutover mechanics, and common reconciliation exceptions.
---

# finance-settlement

| Slug | Title | Description | Edition | Path |
| --- | --- | --- | --- | --- |
| 001-settlement-windows | Settlement windows: trade date vs. settlement date | A securities trade has two dates that matter: the **trade date** (when the buy/sell is agreed) and | 2026-07-25 | references/001-settlement-windows.md |
